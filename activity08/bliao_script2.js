const products = [
    ...
    ['product031_1','Jacket','Sizes XS (0 to 2) S (4 to 6) M (8 to 10) L (12 to 14) XL (16 to 18) XXL (20 to 22). Approx. 46-1/2 Linches','Hotchkiss','Hotchkiss Quilted Jacket',' Quilted longline jacket with shirt tail hem matching corduroy collar and patch pockets. Polyester. Dry clean.',198],
...
function averagePerProduct(productName) {
    ...
    for(let i = 0; i < products.length; i++) {
        if(products[i][1] === productName) { // Check if the product category matches
            sum += products[i][6]; // Price is in the 7th element of each array
            count++;
        }
    }
    return count === 0 ? 0 : (sum / count).toFixed(2); // Calculate average price
}
